...
def cleanup_initial_bons_valides_refuses():
    ...
    push pour tous les bons valides du passe. A partir de la, seuls les bons
    qui passent a valide/refuse APRES le demarrage genereront un push."""
    db = get_db()
    n = 0
    for statut in ("validé", "valide", "refuse", "refusé"):
        try:
...
def search_bons(query: str) -> list[tuple[str, dict]]:
    """Search bons by numero, client, or description. Returns list of (bon_id, bon_data)."""
    db = get_db()
    query_lower = query.lower().strip()
    results = []

    # Get recent bons (last 50 with signatures)
    docs = db.collection(BONS_COLLECTION).order_by(
        "date", direction="DESCENDING"
    ).limit(50).stream()

    for doc in docs:
        data = doc.to_dict()
        # Search in numero, client, description
        numero = str(data.get("numero", "")).lower()
        client = str(data.get("client", "")).lower()
        description = str(data.get("description", "")).lower()
        technicien = str(data.get("technicien", "")).lower()

        if (query_lower in numero or query_lower in client
                or query_lower in description or query_lower in technicien):
